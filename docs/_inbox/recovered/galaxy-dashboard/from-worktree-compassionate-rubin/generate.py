...
GITHUB_ROOT = DEFAULT_REPO.parent
...
def resolve_repo(repo: str | Path | None = None) -> Path:
    """Resolve a repo selector to an absolute BCOS repo path.

    Accepts either an absolute/relative filesystem path or a bare GitHub repo
    folder name. The CLI and HTTP server both use this so `--repo foo` matches
    `?repo=foo`.
    """
    if repo is None or str(repo).strip() == "":
        return DEFAULT_REPO

    raw = Path(str(repo)).expanduser()
    if raw.is_absolute():
        return raw.resolve()
    ...
    return (GITHUB_ROOT / raw).resolve()
