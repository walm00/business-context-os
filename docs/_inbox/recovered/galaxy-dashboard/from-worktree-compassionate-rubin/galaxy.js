...
const PLANET_PICK_RADIUS_PX = 32;
...
function createRawGalaxyGraph(container, modules) {
  ...
  root.add(linkRoot);
  root.add(nodeRoot);
  // The core is the light source. Directionals stay very low as camera fill
  // so the scene does not imply a second off-screen sun.
  scene.add(new THREE.AmbientLight(0xaab7ff, 0.58));
  const key = new THREE.DirectionalLight(0xffffff, 0.055);
  key.position.set(-520, 180, 420);
  scene.add(key);
  const fill = new THREE.DirectionalLight(0x6ec3ff, 0.035);
  fill.position.set(420, -320, 180);
  scene.add(fill);
  buildSkyDome();
  buildStarfield();
  buildNebula();
  ...
  function nodeGeometry(node, radius) {
    ...
    if (node.tier === "ambient" || node.bucket === "_archive") return new THREE.IcosahedronGeometry(radius * 0.92, 1);
    return new THREE.SphereGeometry(radius, Math.max(18, nodeResolutionValue), Math.max(10, Math.floor(nodeResolutionValue / 2)));
  }

  function nodeMaterial(node) {
    if (node.isSun) {
      return makeCoreMaterial();
    }
    const transparent = node.tier === "ambient";
    const base = new THREE.Color(baseNodeColor(node));
    return new THREE.MeshStandardMaterial({
      color: 0xffffff,
      map: makeDocTexture(node, base),
      bumpMap: makeDocBumpTexture(node),
      bumpScale: node.structuralKind === "archive" ? 0.9 : 0.42,
      emissive: base.clone().multiplyScalar(node.tier === "ambient" ? 0.42 : 0.56),
      emissiveIntensity: node.tier === "ambient" ? 0.24 : 0.34,
      roughness: node.structuralKind === "collection" ? 0.58 : 0.86,
      metalness: node.structuralKind === "collection" ? 0.18 : 0.02,
      transparent,
      opacity: transparent ? effectiveNodeOpacity(node) : 1,
    });
  }
    ...
    return new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          vec2 uv = vUv;
          uv.x += time * 0.018;
          float cells = fbm(uv * vec2(18.0, 9.0) + vec2(time * 0.06, -time * 0.025));
          float bands = sin((uv.x + cells * 0.16) * 42.0) * 0.5 + 0.5;
          float heat = clamp(cells * 0.72 + bands * 0.24, 0.0, 1.0);
          float fresnel = pow(1.0 - max(dot(normalize(vNormalW), normalize(vView)), 0.0), 2.35);
          vec3 deep = vec3(1.0, 0.36, 0.05);
          vec3 mid = vec3(1.0, 0.78, 0.20);
          vec3 hot = vec3(1.0, 0.98, 0.68);
          vec3 color = mix(deep, mid, heat);
          color = mix(color, hot, smoothstep(0.58, 1.0, heat));
          color += vec3(1.0, 0.38, 0.08) * fresnel * 0.9;
          color *= 1.0 + heat * 0.45;
  ...
  function updateScreenLabels() {
    ...
    const activePlanet = !activeNode && (state.hoveredPlanet || state.focusPlanet);
    if (state.activeDocLabel && activeNode && activeNode.__threeObj && !hidden) {
      const planetLabel = planetLabelForNode(activeNode);
      state.activeDocLabel.textContent = planetLabel.label;
      state.activeDocLabel.classList.toggle("galaxy-label--selected", state.selectedId === activeNode.id);
      const ok = projectToScreen(planetLabel.x, planetLabel.y, planetLabel.z, rect, -42, state.activeDocLabel);
      state.activeDocLabel.hidden = !ok;
    } else if (state.activeDocLabel && activePlanet && !hidden) {
      state.activeDocLabel.textContent = activePlanet.label;
      state.activeDocLabel.classList.toggle("galaxy-label--selected", !!state.focusPlanet && state.focusPlanet.key === activePlanet.key);
      const ok = projectToScreen(activePlanet.x, activePlanet.y, activePlanet.z, rect, -42, state.activeDocLabel);
      state.activeDocLabel.hidden = !ok;
  ...
  function planetLabelForNode(node) {
    const folder = node.folderGroupKey && state.folderCenters && state.folderCenters.get(node.folderGroupKey);
    if (folder) {
      return { label: folder.label, x: folder.x, y: folder.y, z: folder.z };
    }
    const cluster = clusterFor(node);
    const c = state.clusterCenters && state.clusterCenters.get(cluster);
    if (c) return { label: cluster, x: c.x, y: c.y, z: c.z };
    return { label: cluster || "(unclassified)", x: node.x || 0, y: node.y || 0, z: node.z || 0 };
  }
  ...
  function makeCoreTexture() {
    if (makeCoreTexture.cached) return makeCoreTexture.cached;
    const canvas = document.createElement("canvas");
    canvas.width = 512;
    canvas.height = 256;
    const ctx = canvas.getContext("2d");
    ...
    for (let y = 0; y < canvas.height; y++) {
      for (let x = 0; x < canvas.width; x++) {
        ...
        const band =
          Math.sin(nx * Math.PI * 18 + Math.sin(ny * Math.PI * 9) * 1.8) * 0.5 +
          Math.sin((nx + ny) * Math.PI * 25) * 0.28 +
          Math.sin((nx * 3.7 - ny * 2.2) * Math.PI * 8) * 0.22;
        const limb = Math.pow(Math.sin(ny * Math.PI), 0.22);
        const heat = Math.max(0, Math.min(1, 0.62 + band * 0.22));
        const idx = (y * canvas.width + x) * 4;
        image.data[idx] = Math.round((255 - heat * 24) * limb + 255 * (1 - limb));
        image.data[idx + 1] = Math.round((178 + heat * 58) * limb + 245 * (1 - limb));
        image.data[idx + 2] = Math.round((70 + heat * 58) * limb + 188 * (1 - limb));
    ...
    ctx.globalCompositeOperation = "screen";
    for (let i = 0; i < 56; i++) {
      const x = stableUnit(`core-plage-x:${i}`) * canvas.width;
      const y = stableUnit(`core-plage-y:${i}`) * canvas.height;
      const r = 6 + stableUnit(`core-plage-r:${i}`) * 22;
      const g = ctx.createRadialGradient(x, y, 0, x, y, r);
      g.addColorStop(0, "rgba(255,255,220,0.26)");
    ...
    makeCoreTexture.cached = texture;
    return texture;
  }

  function makeDocTexture(node, baseColor) {
    makeDocTexture.cache = makeDocTexture.cache || new Map();
    const key = `${node.id}:${baseColor.getHexString()}:${node.structuralKind}:${node.tier}`;
    if (makeDocTexture.cache.has(key)) return makeDocTexture.cache.get(key);
    const canvas = document.createElement("canvas");
    canvas.width = 192;
    canvas.height = 96;
    const ctx = canvas.getContext("2d");
    const image = ctx.createImageData(canvas.width, canvas.height);
    const shadow = new THREE.Color(baseColor).multiplyScalar(node.tier === "ambient" ? 0.9 : 0.82);
    const light = new THREE.Color(baseColor).lerp(new THREE.Color("#ffffff"), node.tier === "ambient" ? 0.36 : 0.58);
    const accent = new THREE.Color(baseColor).lerp(new THREE.Color("#111827"), 0.18);
    for (let y = 0; y < canvas.height; y++) {
      for (let x = 0; x < canvas.width; x++) {
        const nx = x / canvas.width;
        const ny = y / canvas.height;
        const terrain =
          Math.sin((nx * 5.0 + stableUnit(`doc-a:${node.id}`)) * Math.PI * 2) * 0.16 +
          Math.sin((ny * 7.0 + stableUnit(`doc-b:${node.id}`)) * Math.PI * 2) * 0.12 +
          stableUnit(`doc-pix:${node.id}:${x >> 2}:${y >> 2}`) * 0.2;
        const lane =
          node.type === "process" ? Math.sin((ny * 8.0 + nx * 1.4) * Math.PI * 2) * 0.12 :
          node.type === "reference" ? Math.sin((nx * 10.0) * Math.PI * 2) * 0.08 :
          0;
        const shade = clamp(0.76 + terrain + lane, 0, 1);
        const color = new THREE.Color().copy(shadow).lerp(light, shade);
        if (stableUnit(`doc-crater:${node.id}:${x >> 3}:${y >> 3}`) > 0.965) color.lerp(accent, 0.32);
        const idx = (y * canvas.width + x) * 4;
    ...
    texture.colorSpace = THREE.SRGBColorSpace;
    texture.wrapS = THREE.RepeatWrapping;
    makeDocTexture.cache.set(key, texture);
    return texture;
  }

  function makeDocBumpTexture(node) {
    makeDocBumpTexture.cache = makeDocBumpTexture.cache || new Map();
    if (makeDocBumpTexture.cache.has(node.id)) return makeDocBumpTexture.cache.get(node.id);
    const canvas = document.createElement("canvas");
    canvas.width = 128;
    canvas.height = 64;
    const ctx = canvas.getContext("2d");
    ...
    for (let y = 0; y < canvas.height; y++) {
      for (let x = 0; x < canvas.width; x++) {
        const v = Math.round(108 + stableUnit(`bump:${node.id}:${x >> 1}:${y >> 1}`) * 72);
        const idx = (y * canvas.width + x) * 4;
        image.data[idx] = v;
        image.data[idx + 1] = v;
        image.data[idx + 2] = v;
    ...
    const texture = new THREE.CanvasTexture(canvas);
    texture.wrapS = THREE.RepeatWrapping;
    makeDocBumpTexture.cache.set(node.id, texture);
    return texture;
  }

  function makePlanetTexture(name, colorValue) {
    makePlanetTexture.cache = makePlanetTexture.cache || new Map();
    const key = `${name}:${colorValue.getHexString()}`;
    if (makePlanetTexture.cache.has(key)) return makePlanetTexture.cache.get(key);
    const canvas = document.createElement("canvas");
    canvas.width = 384;
    canvas.height = 192;
    const ctx = canvas.getContext("2d");
    const image = ctx.createImageData(canvas.width, canvas.height);
    const sea = new THREE.Color(colorValue).multiplyScalar(0.9);
    const land = new THREE.Color(colorValue).lerp(new THREE.Color("#f5e7bd"), 0.56);
    const ridge = new THREE.Color(colorValue).lerp(new THREE.Color("#ffffff"), 0.7);
    for (let y = 0; y < canvas.height; y++) {
      for (let x = 0; x < canvas.width; x++) {
        const nx = x / canvas.width;
        const ny = y / canvas.height;
        const bands = Math.sin((ny * 9.0 + stableUnit(`planet-band:${name}`) * 2.0) * Math.PI) * 0.18;
        const continents =
          stableUnit(`planet-cell:${name}:${x >> 4}:${y >> 4}`) * 0.44 +
          Math.sin((nx * 4.0 + ny * 2.0 + stableUnit(`planet-wave:${name}`)) * Math.PI * 2) * 0.22;
        const k = clamp(0.44 + bands + continents, 0, 1);
        const color = new THREE.Color().copy(sea).lerp(land, k);
        if (Math.abs(bands) > 0.15 && stableUnit(`planet-ridge:${name}:${x >> 2}:${y >> 2}`) > 0.68) {
          color.lerp(ridge, 0.18);
        }
        const idx = (y * canvas.width + x) * 4;
        image.data[idx] = Math.round(color.r * 255);
        image.data[idx + 1] = Math.round(color.g * 255);
    ...
    texture.colorSpace = THREE.SRGBColorSpace;
    texture.wrapS = THREE.RepeatWrapping;
    makePlanetTexture.cache.set(key, texture);
    return texture;
  }

  function makeCoronaTexture() {
    if (makeCoronaTexture.cached) return makeCoronaTexture.cached;
    const canvas = document.createElement("canvas");
    canvas.width = 512;
    canvas.height = 512;
    const ctx = canvas.getContext("2d");
    ...
    base.addColorStop(0, "rgba(255,255,230,0.92)");
    base.addColorStop(0.18, "rgba(255,222,128,0.62)");
    base.addColorStop(0.42, "rgba(255,126,42,0.22)");
    base.addColorStop(0.72, "rgba(96,148,255,0.08)");
    ...
    ctx.globalCompositeOperation = "screen";
    for (let i = 0; i < 96; i++) {
      const angle = stableUnit(`corona-ray-a:${i}`) * Math.PI * 2;
      const spread = 0.012 + stableUnit(`corona-ray-s:${i}`) * 0.028;
      const length = 92 + stableUnit(`corona-ray-l:${i}`) * 176;
      const inner = 48 + stableUnit(`corona-ray-i:${i}`) * 34;
      const alpha = 0.018 + stableUnit(`corona-ray-o:${i}`) * 0.052;
      const ax = Math.cos(angle);
      const ay = Math.sin(angle);
      const px = -ay;
      const py = ax;
      ctx.beginPath();
      ctx.moveTo(cx + ax * inner + px * inner * spread, cy + ay * inner + py * inner * spread);
      ctx.lineTo(cx + ax * (inner + length), cy + ay * (inner + length));
      ctx.lineTo(cx + ax * inner - px * inner * spread, cy + ay * inner - py * inner * spread);
      ctx.closePath();
      ctx.fillStyle = `rgba(255,205,102,${alpha})`;
      ctx.fill();
    }
    ctx.globalCompositeOperation = "source-over";
    ...
    const texture = new THREE.CanvasTexture(canvas);
    texture.colorSpace = THREE.SRGBColorSpace;
    makeCoronaTexture.cached = texture;
    return texture;
  }

  function makeFlareTexture() {
    if (makeFlareTexture.cached) return makeFlareTexture.cached;
    const canvas = document.createElement("canvas");
    canvas.width = 512;
    canvas.height = 128;
    const ctx = canvas.getContext("2d");
    const streak = ctx.createLinearGradient(0, 64, 512, 64);
    streak.addColorStop(0, "rgba(255,110,32,0)");
    streak.addColorStop(0.28, "rgba(255,158,57,0.18)");
    streak.addColorStop(0.5, "rgba(255,255,215,0.62)");
    streak.addColorStop(0.72, "rgba(255,158,57,0.18)");
    streak.addColorStop(1, "rgba(255,110,32,0)");
    ctx.fillStyle = streak;
    ctx.fillRect(0, 0, 512, 128);
    const core = ctx.createRadialGradient(256, 64, 0, 256, 64, 70);
    core.addColorStop(0, "rgba(255,255,240,0.86)");
    core.addColorStop(0.5, "rgba(255,184,77,0.22)");
    core.addColorStop(1, "rgba(255,128,32,0)");
    ctx.globalCompositeOperation = "screen";
    ctx.fillStyle = core;
    ctx.fillRect(160, 0, 192, 128);
    const mask = ctx.createLinearGradient(0, 0, 0, 128);
    mask.addColorStop(0, "rgba(0,0,0,0)");
    mask.addColorStop(0.22, "rgba(0,0,0,0.38)");
    mask.addColorStop(0.5, "rgba(0,0,0,1)");
    mask.addColorStop(0.78, "rgba(0,0,0,0.38)");
    ...
    const texture = new THREE.CanvasTexture(canvas);
    texture.colorSpace = THREE.SRGBColorSpace;
    makeFlareTexture.cached = texture;
    return texture;
  }
  ...
  function buildGalaxyDecorations() {
    ...
    for (const [name, c] of state.clusterCenters) {
      ...
      solarRing.rotation.z = c.angle * 0.42;
      solarRing.scale.y = 0.64 + stableUnit(`orbit-squash:${name}`) * 0.22;
      ...
      const body = new THREE.Mesh(
        new THREE.SphereGeometry(c.planetRadius, 48, 24),
        new THREE.MeshStandardMaterial({
          color: 0xffffff,
          map: makePlanetTexture(name, color),
      ...
      body.rotation.y = stableUnit(`planet-rot:${name}`) * Math.PI * 2;
      const atmosphere = new THREE.Mesh(
        new THREE.SphereGeometry(c.planetRadius * 1.15, 36, 18),
      ...
      const folders = [...(state.folderCenters || new Map()).values()]
        .filter((f) => f.cluster === name)
        .sort((a, b) => a.label.localeCompare(b.label));
      for (const f of folders) {
        const folderColor = new THREE.Color(f.color || c.color || "#6ec3ff");
        const folderPlanet = new THREE.Group();
        folderPlanet.position.set(f.x, f.y, f.z);
        folderPlanet.userData.folderGroup = f.key;
        folderPlanet.userData.folderLabel = f.label;
        const folderBody = new THREE.Mesh(
          new THREE.SphereGeometry(f.radius, 32, 16),
          new THREE.MeshStandardMaterial({
            color: 0xffffff,
            map: makePlanetTexture(f.label, folderColor),
            emissive: folderColor.clone().multiplyScalar(0.48),
            emissiveIntensity: 0.28,
            roughness: 0.78,
            metalness: 0.04,
          }),
        );
        folderBody.rotation.y = stableUnit(`folder-planet-rot:${f.key}`) * Math.PI * 2;
        folderPlanet.add(folderBody);
        decorRoot.add(folderPlanet);

        const folderOrbit = makeRing(f.docOrbitRadius, folderColor, f.count >= 4 ? 0.075 : 0.045, 96);
        folderOrbit.position.set(f.x, f.y, f.z);
        folderOrbit.rotation.x = f.tilt;
        folderOrbit.rotation.z = f.phase;
        folderOrbit.scale.y = 0.62;
        folderOrbit.userData.folderOrbitRing = true;
        folderOrbit.userData.folderGroup = f.key;
        folderOrbit.userData.baseOpacity = f.count >= 4 ? 0.075 : 0.045;
        decorRoot.add(folderOrbit);
      }
  ...
  function attachRawHalo(group, node, radius) {
    if (node.isSun) {
      const coreLight = new THREE.PointLight(0xffcf73, 5.4, 1450, 1.16);
      coreLight.userData.coreLight = true;
      group.add(coreLight);
      addCoreSprite(group, radius, 9.8, makeCoronaTexture(), 0xffffff, 0.34, "coronaSprite");
      addCoreSprite(group, radius, 15.5, makeCoronaTexture(), 0x7ca8ff, 0.06, "outerCoronaSprite");
      for (let i = 0; i < 4; i++) {
        const flare = addCoreSprite(group, radius, 5.2 + i * 1.12, makeFlareTexture(), 0xffd48a, 0.18 - i * 0.026, "flareSprite");
        flare.material.rotation = stableUnit(`flare-rot:${i}`) * Math.PI * 2;
        flare.userData.phase = stableUnit(`flare-phase:${i}`) * Math.PI * 2;
        flare.userData.spin = (i % 2 === 0 ? 1 : -1) * (0.00008 + stableUnit(`flare-spin:${i}`) * 0.00012);
      }
      group.userData.haloAttached = true;
      return;
    }
    if (node.structuralKind === "planned" || node.structuralKind === "archive") {
      addHalo(group, node, radius, 1.18, 0xffffff, 0.035, null);
    }
    group.userData.haloAttached = true;
  }

  function addCoreSprite(group, radius, scale, texture, color, opacity, kind) {
    const sprite = new THREE.Sprite(new THREE.SpriteMaterial({
      map: texture,
      color,
      transparent: true,
    ...
    sprite.userData.coreSprite = kind;
  ...
  function addHalo(group, node, radius, scale, color, opacity, coronaShell) {
    const halo = new THREE.Mesh(
      new THREE.SphereGeometry(radius, coronaShell ? 48 : 16, coronaShell ? 24 : 8),
      new THREE.MeshBasicMaterial({
        color,
        transparent: true,
        opacity,
        depthWrite: false,
        depthTest: !coronaShell,
        blending: THREE.AdditiveBlending,
        toneMapped: false,
      }),
    );
  ...
  function effectiveNodeOpacity(node) {
    if (!shouldShowNode(node)) return 0;
  ...
  function syncRawMeshes() {
    for (const node of data.nodes) {
      ...
      if (!obj) continue;
      obj.visible = shouldShowNode(node);
      obj.position.set(node.x || 0, node.y || 0, node.z || 0);
      const body = obj.userData.body;
      if (body?.material) {
        if (body.material.color) body.material.color.set(node.isSun ? "#ffffff" : nodeColorAccessor(node));
        if (body.material.uniforms?.time) body.material.uniforms.time.value = performance.now() / 1000;
        if (body.material.transparent) body.material.opacity = effectiveNodeOpacity(node);
        if (body.material.emissive && !node.isSun) {
          body.material.emissive.copy(new THREE.Color(nodeColorAccessor(node)).multiplyScalar(node.tier === "ambient" ? 0.42 : 0.56));
        }
        if (body.material.emissiveIntensity != null) {
          body.material.emissiveIntensity = node.isSun ? 0 : (isInSpotlight(node.id) ? 0.46 : 0.34);
        }
      }
  ...
  function syncDecorationVisibility() {
    ...
    for (const child of decorRoot.children) {
      ...
      if (child.userData?.localOrbitRing) {
      ...
      if (child.userData?.folderOrbitRing) {
        const planetFocused = state.focusPlanet?.kind === "folder" && state.focusPlanet.key === child.userData.folderGroup;
        const planetHovered = state.hoveredPlanet?.kind === "folder" && state.hoveredPlanet.key === child.userData.folderGroup;
        const nodeFocused = focusNode && focusNode.folderGroupKey === child.userData.folderGroup;
        const visible = planetFocused || planetHovered || nodeFocused;
        child.material.opacity = visible ? child.userData.baseOpacity : 0.012;
      }
    }
  }

  function animateCore(now) {
    const core = data.nodes.find((n) => n.isSun && n.__threeObj);
    if (!core) return;
    const obj = core.__threeObj;
    obj.rotation.y += 0.0014;
    obj.rotation.x = Math.sin(now / 6200) * 0.045;
    const pulse = 0.5 + Math.sin(now / 1850) * 0.5;
    for (const child of obj.children) {
      if (child.userData?.coreLight) {
        child.intensity = 5.0 + pulse * 1.2;
      }
      if (child.userData?.coreSprite && child.material) {
        const phase = child.userData.phase || 0;
        const flicker =
          0.82 +
          Math.sin(now / 1400 + phase) * 0.12 +
          Math.sin(now / 390 + phase * 1.7) * 0.04;
        child.material.opacity = child.userData.baseOpacity * flicker;
        const s = child.userData.baseScale * (0.96 + Math.sin(now / 2400 + phase) * 0.035);
        child.scale.set(s, s, 1);
        if (child.userData.coreSprite === "flareSprite") {
          child.material.rotation += child.userData.spin || 0.0001;
        }
      }
    }
  }
...
function atlasToGraphData(atlas) {
  ...
  const nodes = atlas.docs.map((d) => {
    ...
    const cluster = docClusterName(d);
    const isSun = d.path === state.sunId;
    const structuralKind = structuralKindForPath(d.path, d);
    const folderGroup = folderGroupForDoc(d);
    const folderKey = folderGroupKeyFor(cluster, folderGroup);
    return {
      id: d.path,
      name: d.name,
      type: d.type,
      structuralKind,
      cluster,
      folderGroupKey: folderKey,
      folderGroupLabel: folderGroup.label,
      folderGroupKind: folderGroup.kind,
      bucket: d.bucket,
      status: d.status,
      version: d.version,
      age_days: d.age_days,
...
function docClusterName(doc = {}) {
  if (doc.cluster) return doc.cluster;
  const p = String(doc.path || "").replace(/\\/g, "/");
  if (p.startsWith("docs/operations/engagements/")) return "Client Engagements";
  if (p.startsWith("docs/client-delivery/")) return "Operations";
  if (p.startsWith("docs/operations/")) return "Operations";
  if (p.startsWith("docs/finance/")) return "Sales & GTM";
  if (p === "docs/table-of-context.md" || p === "docs/current-state.md" || p === "docs/document-index.md") return "System";
  return doc.bucket !== "active" ? `(${doc.bucket})` : "(unclassified)";
}
...
function folderGroupForDoc(doc = {}) {
  ...
  if (parts[0] !== "docs") {
    return { key: `root:${parts[0] || "(root)"}`, label: parts[0] || "(root)", kind: "root" };
  }

  if (parts[1] === "operations" && parts[2] === "engagements" && parts[3]) {
    const slug = parts[3];
    if (slug === "_template") return { key: "engagement-template", label: "Engagement Template", kind: "template" };
    return { key: `engagement:${slug}`, label: engagementLabel(slug), kind: "engagement" };
  }
  if (parts[1] === "client-delivery") {
    return { key: "folder:client-delivery", label: "Client Delivery", kind: "folder" };
  }
  if (parts[1] === "operations" && parts[2] === "service-profiles") {
    return { key: "folder:service-profiles", label: "Service Profiles", kind: "folder" };
  }
  if (parts[1] === "operations" && parts[2]) {
    return { key: `folder:operations/${parts[2]}`, label: titleizeSlug(parts[2]), kind: "folder" };
  }
  if (parts[1]) {
    return { key: `folder:${parts[1]}`, label: titleizeSlug(parts[1]), kind: "folder" };
  }
  return { key: "folder:docs-root", label: "Docs Root", kind: "folder" };
}

function folderGroupKeyFor(cluster, folderGroup = {}) {
  return `${cluster || "(unclassified)"}::${folderGroup.key || "folder:(unknown)"}`;
}

function engagementLabel(slug) {
  const known = {
    "alphalane": "AlphaLane",
    "gravity-laura": "Gravity / Laura",
    "avk-jurgis": "AVK / Jurgis",
    "metup": "Metup",
    "tystiq": "TYSTIQ",
  };
  return known[slug] || titleizeSlug(slug);
}
...
function buildLayoutState(atlas) {
  ...
  return {
    ...
    folderCenters: state.folderCenters || new Map(),
...
function layoutCluster(data) {
  ...
  for (const [cluster, nodes] of byCluster) {
    ...
    for (const n of nodes) {
      const f = state.folderCenters && state.folderCenters.get(n.folderGroupKey);
      const key = f ? n.folderGroupKey : "__direct";
    ...
    const directNodes = grouped.get("__direct") || [];
    const directCount = directNodes.length || 1;
    directNodes.forEach((n, i) => {
      ...
      const phase = c.phase + (i / directCount) * Math.PI * 2;
      const lane = Math.floor(i / Math.max(1, Math.ceil(directCount / 3)));
    ...
    for (const [folderKey, groupNodes] of grouped) {
      if (folderKey === "__direct") continue;
      const f = state.folderCenters && state.folderCenters.get(folderKey);
      if (!f) continue;
      const count = groupNodes.length || 1;
      groupNodes.forEach((n, i) => {
        const phase = f.phase + (i / count) * Math.PI * 2;
        const lane = Math.floor(i / Math.max(1, Math.ceil(count / 2)));
        const moonRadius = f.docOrbitRadius * (0.78 + lane * 0.18 + stableUnit(`folder-moon:${n.id}`) * 0.12);
        const px = Math.cos(phase) * moonRadius;
        const py = Math.sin(phase) * moonRadius * Math.cos(f.tilt);
        const pz = Math.sin(phase) * moonRadius * Math.sin(f.tilt);
        n.x = f.x + px;
        n.y = f.y + py;
        n.z = f.z + pz + stableJitter(n.id, "folder-z", 10);
      });
    }
  }
}
...
function onCanvasPointerMove(e) {
  ...
  const hit = pickSceneAt(e.clientX, e.clientY);
  if (hit?.type === "planet") {
    if (!state.hoveredPlanet || state.hoveredPlanet.key !== hit.planet.key || state.hoveredId) {
      onPlanetHover(hit.planet);
    }
    return;
  }
  if (hit?.type === "node") {
    if (hit.node.id !== state.hoveredId || state.hoveredPlanet) {
      onNodeHover(hit.node);
    }
    return;
  }
  if (state.hoveredId || state.hoveredPlanet) {
    onNodeHover(null);
  }
}
...
function onCanvasPointerUp(e) {
  ...
  if (wasDrag) return;
  const hit = pickSceneAt(e.clientX, e.clientY);
  if (hit?.type === "planet") onPlanetClick(hit.planet);
  else if (hit?.type === "node") onNodeClick(hit.node);
  else onBackgroundClick();
}

function pickSceneAt(clientX, clientY) {
  const planet = pickPlanetAt(clientX, clientY);
  if (planet) return { type: "planet", planet };
...
function pickPlanetAt(clientX, clientY) {
  ...
  const camera = state.graph.camera && state.graph.camera();
  const canvas = getGraphCanvas();
  if (!camera || !canvas || !camera.position?.clone) return null;
  const rect = canvas.getBoundingClientRect();
  const x = clientX - rect.left;
  const y = clientY - rect.top;
  if (x < 0 || y < 0 || x > rect.width || y > rect.height) return null;
  ...
  let best = null;
  for (const planet of pickablePlanets()) {
    const projected = camera.position.clone();
    projected.set(planet.x || 0, planet.y || 0, planet.z || 0);
    if (typeof projected.project !== "function") continue;
    projected.project(camera);
    if (projected.z < -1 || projected.z > 1) continue;
    const sx = (projected.x + 1) * rect.width / 2;
    const sy = (-projected.y + 1) * rect.height / 2;
    const radius = planet.kind === "cluster"
      ? Math.max(PLANET_PICK_RADIUS_PX, Math.min(52, (planet.radius || 18) * 1.5))
      : Math.max(20, Math.min(42, (planet.radius || 8) * 2.2));
    const dist = Math.hypot(sx - x, sy - y);
    if (dist > radius) continue;
    if (!best || dist < best.dist) best = { ...planet, dist, sx, sy };
  }
  return best;
}

function pickablePlanets() {
  const planets = [];
  for (const [key, f] of state.folderCenters || new Map()) {
    planets.push({
      kind: "folder",
      key,
      label: f.label,
      ...
      z: f.z,
      radius: f.radius,
      docOrbitRadius: f.docOrbitRadius,
    });
  }
  for (const [key, c] of state.clusterCenters || new Map()) {
    planets.push({
      kind: "cluster",
      key,
      label: key,
      ...
      y: c.y,
      z: c.z,
      radius: c.planetRadius,
      docOrbitRadius: c.localRadius,
    });
  }
  return planets;
}
...
const FOLDER_PLANET_PALETTE = [
  "#ffb86c", "#6ec3ff", "#b48cff", "#7ad7c1",
  "#ff6b8a", "#9bff9b", "#ffe066", "#f08cff",
];

const FOLDER_PLANET_MIN_DOCS = 4;
const FOLDER_DETAIL_COLLAPSE_DOCS = 10;

function computeClusterCenters(atlas) {
  const domains = [...new Set(
    (atlas.docs || []).map((d) => docClusterName(d))
  )].sort();
  const N = domains.length || 1;
  state.clusterCenters = new Map();
  domains.forEach((name, i) => {
    const docs = (atlas.docs || []).filter((d) =>
      docClusterName(d) === name
    );
...
function computeFolderCenters(atlas) {
  const groups = new Map();
  for (const d of (atlas.docs || [])) {
    if (d.path === state.sunId) continue;
    const cluster = docClusterName(d);
    const folderGroup = folderGroupForDoc(d);
    const key = folderGroupKeyFor(cluster, folderGroup);
    if (!groups.has(key)) {
      groups.set(key, {
        key,
        sourceKey: folderGroup.key,
        label: folderGroup.label,
        kind: folderGroup.kind,
        cluster,
        docs: [],
      });
    }
  ...
  for (const group of groups.values()) {
    if (!shouldRenderFolderPlanet(group.kind, group.docs.length)) continue;
    if (!byCluster.has(group.cluster)) byCluster.set(group.cluster, []);
    byCluster.get(group.cluster).push(group);
  }

  state.folderCenters = new Map();
  for (const [cluster, clusterGroups] of byCluster) {
    const c = state.clusterCenters && state.clusterCenters.get(cluster);
    if (!c) continue;
    clusterGroups.sort((a, b) => {
      const ak = a.kind === "engagement" ? 0 : 1;
      const bk = b.kind === "engagement" ? 0 : 1;
      if (ak !== bk) return ak - bk;
      if (b.docs.length !== a.docs.length) return b.docs.length - a.docs.length;
      return a.label.localeCompare(b.label);
    });
    const slots = clusterGroups.length || 1;
    clusterGroups.forEach((group, i) => {
      const count = group.docs.length || 1;
      const lane = Math.floor(i / 6);
      const phaseOffset = stableJitter(`folder-angle:${group.key}`, "phase", 0.18);
      const angle = c.phase + ((i + 0.35) / slots) * Math.PI * 2 + phaseOffset;
      const distance = c.localRadius * (0.56 + lane * 0.24) + (group.kind === "engagement" ? 12 : 0);
      const tilt = c.tilt + stableJitter(`folder-tilt:${group.key}`, "tilt", 0.42);
      const px = Math.cos(angle) * distance;
      const py = Math.sin(angle) * distance * Math.cos(tilt);
      const pz = Math.sin(angle) * distance * Math.sin(tilt);
      const color = FOLDER_PLANET_PALETTE[(i + Math.floor(stableUnit(`folder-color:${cluster}`) * 7)) % FOLDER_PLANET_PALETTE.length];
      state.folderCenters.set(group.key, {
        key: group.key,
        ...
        x: c.x + px,
        y: c.y + py,
        z: c.z + pz + stableJitter(`folder-z:${group.key}`, "z", 24),
        color,
        radius: Math.max(5.5, Math.min(17, 5.4 + Math.sqrt(count) * 1.55)),
        docOrbitRadius: Math.max(24, Math.min(66, 20 + Math.sqrt(count) * 5.3)),
        tilt,
        phase: stableUnit(`folder-phase:${group.key}`) * Math.PI * 2,
      });
    });
  }
}

function shouldRenderFolderPlanet(kind, count) {
  if (kind === "template") return false;
  if (kind === "engagement") return true;
  return count >= FOLDER_PLANET_MIN_DOCS;
}
...
function shouldShowNode(node) {
  ...
  if (node.isSun) return true;
  const folder = node.folderGroupKey && state.folderCenters && state.folderCenters.get(node.folderGroupKey);
  if (!folder || folder.count < FOLDER_DETAIL_COLLAPSE_DOCS) return true;
  if (state.focusPlanet?.kind === "folder" && state.focusPlanet.key === node.folderGroupKey) return true;
  if (state.focusPlanet?.kind === "cluster" && state.focusPlanet.key === clusterFor(node)) return true;
  if (state.hoveredPlanet?.kind === "folder" && state.hoveredPlanet.key === node.folderGroupKey) return true;
  if (state.hoveredPlanet?.kind === "cluster" && state.hoveredPlanet.key === clusterFor(node)) return true;
  if (state.hoveredId) {
    const hovered = getNodeById(state.hoveredId);
    if (hovered?.folderGroupKey === node.folderGroupKey) return true;
  }
  if (state.selectedId) {
    const selected = getNodeById(state.selectedId);
    if (selected?.folderGroupKey === node.folderGroupKey) return true;
  }
  return false;
}

function idsForPlanet(planet) {
  const data = state.graph?.graphData?.();
  const ids = new Set();
  if (!data?.nodes || !planet) return ids;
  for (const node of data.nodes) {
    if (planet.kind === "folder" && node.folderGroupKey === planet.key) ids.add(node.id);
    if (planet.kind === "cluster" && clusterFor(node) === planet.key) ids.add(node.id);
  }
  return ids;
}
...
function onNodeHover(node) {
  ...
  const changed = state.hoveredId !== nextId || !!state.hoveredPlanet;
...
function onPlanetHover(planet) {
  const changed = state.hoveredPlanet?.key !== planet?.key || state.hoveredId !== null;
  state.hoveredId = null;
  state.hoveredPlanet = planet || null;
  if (changed) refreshGraphVisuals();
  if (!planet) {
    state.tooltipEl.style.display = "none";
    document.body.style.cursor = "";
  ...
  document.body.style.cursor = "pointer";
  const kind = planet.kind === "folder" ? "folder / engagement planet" : "domain planet";
  const docText = planet.count === 1 ? "1 doc" : `${planet.count || 0} docs`;
  const detail = planet.kind === "folder"
    ? `${escapeHtml(planet.cluster || "(unclassified)")} · ${docText}`
    : `${docText} · click to focus domain`;
  state.tooltipEl.innerHTML = `
    <div class="galaxy-tooltip-title">${escapeHtml(planet.label)}</div>
    <div class="galaxy-tooltip-meta">${kind}</div>
    <div class="galaxy-tooltip-meta">${detail}</div>
  `;
  state.tooltipEl.style.display = "block";
}
...
function onPlanetClick(planet) {
  ...
  state.neighborCursor = -1;
  state.spotlightSet = idsForPlanet(planet);
  state.navTrail = [];
  renderTrail();
  closeDrawer();
  refreshGraphVisuals();
  flyCameraToPlanet(planet);
  status.ok(`focused planet: ${planet.label}`);
}

function focusPlanetForNode(node) {
  if (!node || !node.folderGroupKey) return;
  const folder = state.folderCenters && state.folderCenters.get(node.folderGroupKey);
  if (!folder) return;
  state.focusPlanet = {
    kind: "folder",
    key: folder.key,
    ...
    docOrbitRadius: folder.docOrbitRadius,
  };
}
...
function flyCameraToNode(node, duration = 850) {
  ...
  flyCameraToPoint(node, endDistance, duration);
}

function flyCameraToPlanet(planet, duration = 850) {
  const distance = planet.kind === "cluster"
    ? Math.max(210, (planet.docOrbitRadius || 80) * 2.4)
    : Math.max(105, (planet.docOrbitRadius || 42) * 2.7);
  flyCameraToPoint(planet, distance, duration);
}

function flyCameraToPoint(point, endDistance, duration = 850) {
