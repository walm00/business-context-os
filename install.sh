...
echo ""
echo -e "${BLUE}Business Context OS - Installer${NC}"
echo "================================"
echo ""
...
if [ -f "$TARGET_DIR/install.sh" ] && grep -q "Business Context OS" "$TARGET_DIR/install.sh" 2>/dev/null; then
...
# Settings (only if no settings exist at all)
copy_if_missing "$SCRIPT_DIR/.claude/settings.local.json" ".claude/settings.local.json"
...
echo "     Ask Claude: \"Scan my repo and create a Table of Context\""
echo ""
