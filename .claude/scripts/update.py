...
def ensure_python_shim(project_root: str) -> list:
    """Create .claude/bin/python3 (POSIX) and .claude/bin/python3.cmd (Windows)
    shims if missing or stale. Returns list of what was created/refreshed.

    Why: Windows `python3` resolves to the Microsoft Store stub which exits 49
    without running. Hooks, settings.json hook commands, and scheduled-task
    Bash invocations that hardcoded `python3` all failed silently. The shim
    bypasses the stub by using `py -3` (Windows Python Launcher). POSIX shim
    is a passthrough so callers never branch on OS.
    """
    created = []
    bin_dir = os.path.join(project_root, ".claude", "bin")
    os.makedirs(bin_dir, exist_ok=True)

    # Windows shim
    win_shim_path = os.path.join(bin_dir, "python3.cmd")
    win_shim_content = (
        "@echo off\r\n"
        "py -3 %*\r\n"
        "exit /b %ERRORLEVEL%\r\n"
    ...
    win_existing = ""
    if os.path.exists(win_shim_path):
        try:
            with open(win_shim_path, "rb") as f:
                win_existing = f.read().decode("utf-8", errors="replace")
        except Exception:
            win_existing = ""
    if win_existing.strip() != win_shim_content.strip():
        with open(win_shim_path, "wb") as f:
            f.write(win_shim_content.encode("utf-8"))
        created.append("  ~ .claude/bin/python3.cmd")

    # POSIX shim — also handles Windows Git Bash by preferring `py -3` if
    # available (Python Launcher is never aliased to the MS Store stub).
    # On macOS/Linux, `py` doesn't exist and we fall through to env python3.
    posix_shim_path = os.path.join(bin_dir, "python3")
    posix_shim_content = (
        "#!/usr/bin/env bash\n"
    ...
    posix_existing = ""
    if os.path.exists(posix_shim_path):
        try:
            with open(posix_shim_path, "r", encoding="utf-8") as f:
                posix_existing = f.read()
        except Exception:
            posix_existing = ""
    if posix_existing.strip() != posix_shim_content.strip():
        with open(posix_shim_path, "w", encoding="utf-8", newline="\n") as f:
            f.write(posix_shim_content)
        try:
            os.chmod(posix_shim_path, 0o755)
        except Exception:
            pass  # Windows filesystems ignore chmod; fine
        created.append("  ~ .claude/bin/python3")
...
def main():
    ...
    def run_scan(upstream_root: str):
        ...
        if total == 0 and not review_files and not merge_files:
            # Even with no framework-file drift, the Python shim may be
            # missing — happens on every repo installed before the shim
            # feature shipped. Run shim convergence unconditionally so
            # existing installs heal on the next update run.
            shim_created = ensure_python_shim(str(local_root))
            if shim_created:
                print("  Python shim:")
                for item in shim_created:
                    print(item)
                print()
        ...
        # ----------------------------------------- Python shim (Win/macOS fix)
        # Ensures .claude/bin/python3 + .claude/bin/python3.cmd exist so
        # BCOS-controlled callers (hooks, settings.json hook commands, codex
        # hooks) can invoke Python via the shim instead of bare `python3`.
        # On Windows this bypasses the MS Store stub; on POSIX it's a
        # transparent passthrough. Idempotent.
        shim_created = ensure_python_shim(str(local_root))
        if shim_created:
            print(f"  Python shim:")
            for item in shim_created:
