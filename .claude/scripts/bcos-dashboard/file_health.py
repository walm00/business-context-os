...
from atlas_ingest import iter_docs, parse_frontmatter  # noqa: E402
...
# Frontmatter regex retained here for the auto-fix writer (`fix_missing_last_updated`)
# which mutates the raw frontmatter block. The reader path uses atlas_ingest.parse_frontmatter.
...
def _iter_docs() -> list[Path]:
    """Doc iterator with file_health's exclusions. Delegates to atlas_ingest."""
    return iter_docs(skip_dirs=SKIP_DIRS, include_dotfiles=False)


def _parse_frontmatter(text: str) -> dict | None:
    """Parse frontmatter. Delegates to atlas_ingest's superset parser.

    file_health only inspects scalar required fields, but the shared parser
    also returns lists for `tags`/`depends-on`/`consumed-by` — those keys are
    just ignored here, so behavior for the required-field check is unchanged.
    """
