...
# summary sentence + a 5-dot maintenance strip. All the data is
...
def _headline_for_state(action_count: int, overall_sev: str, freshness_sev: str,
                       freshness_value: str | None, first_run: bool = False) -> tuple[str, str]:
    ...
    """
    if first_run:
        return ("Welcome to BCOS. Your first maintenance check runs tomorrow "
                "at 9 AM. Come back to see what it finds.", "first_run")
...
def collect_cockpit() -> dict:
    ...
    # Pull per-job data for the 5-dot strip (human label + verdict glyph + severity).
    dots = []
    worst_sev = "ok"
